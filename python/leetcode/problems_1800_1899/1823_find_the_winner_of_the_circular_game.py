...
# Solution: https://youtu.be/PBBQgW_75e0
# Credit: Navdeep Singh founder of NeetCode
def find_the_winner(n, k):
    res = 0
    for people in range(1, n + 1):
        res = (res + k) % people
    # The result is 0-indexed, so we add 1 for a 1-indexed answer
    return res + 1
    # Time: O(n)
    # Space: O(1)
...
def main():
    result = find_the_winner(n = 5, k = 2)
    print(result) # 3

    result = find_the_winner(n = 6, k = 5)
    print(result) # 1
