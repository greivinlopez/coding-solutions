...
# choose any bit (including any leading zeros not shown) and flip it. We can flip
# the first bit from the right to get 110, flip the second bit from the right to
...
# Solution: https://youtu.be/yz48myznqQY
...
def min_bit_flips(start, goal):
    ...
    while n:
        ...
        res += 1
    return res
    # Time: O(k) k is the number of differing bits between the start and goal integers.
    # Space: O(1)


def main():
    result = min_bit_flips(start = 10, goal = 7)
    print(result) # 3

    result = min_bit_flips(start = 3, goal = 4)
    print(result) # 3
