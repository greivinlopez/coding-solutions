...
# Solution: https://youtu.be/X9QtxzsAsYo
...
def check_valid_cuts(n, rectangles):
    ...
    # Time: O(m * log(m))  m = length of rectangles 
    # Space: O(m)


def main():
    result = check_valid_cuts(n = 5, rectangles = [[1,0,5,2],[0,2,2,4],[3,2,5,3],[0,4,4,5]])
    print(result) # True

    result = check_valid_cuts(n = 4, rectangles = [[0,0,1,1],[2,0,3,4],[0,2,2,3],[3,0,4,3]])
    print(result) # True

    result = check_valid_cuts(n = 4, rectangles = [[0,2,2,4],[1,0,3,2],[2,2,3,4],[3,0,4,2],[3,2,4,4]])
    print(result) # False
