...
# Solution: https://algo.monster/liteproblems/1387
# Credit: AlgoMonster
def get_kth(lo, hi, k):
    ...
    # Generate all integers in the range [lo, hi]
    numbers = list(range(lo, hi + 1))
    
    # Sort by power value (and implicitly by the number itself for ties)
    sorted_numbers = sorted(numbers, key=calculate_power)
    
    # Return the k-th element (convert from 1-indexed to 0-indexed)
    return sorted_numbers[k - 1]
    # Time: O(n * log n * M)
    # Space: O(n)
    # M = the maximum number of steps to reach 1 in the Collatz sequence


def main():
    result = get_kth(lo = 12, hi = 15, k = 2)
    print(result) # 13

    result = get_kth(lo = 7, hi = 11, k = 4)
    print(result) # 7
