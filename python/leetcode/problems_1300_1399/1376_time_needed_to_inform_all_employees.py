...
# Solution: https://youtu.be/zdBYi0p4L5Q
...
def num_of_minutes(n, head_id, manager, inform_time):
    ...
    while q:
        ...
        res = max(res, time)
        for emp in adj[i]:
            q.append((emp, time + inform_time[i]))
...
def main():
    result = num_of_minutes(n = 1, head_id = 0, manager = [-1], inform_time = [0])
    print(result) # 0

    result = num_of_minutes(n = 6, head_id = 2, manager = [2,2,-1,2,2,2], inform_time = [0,0,1,0,0,0])
    print(result) # 1
