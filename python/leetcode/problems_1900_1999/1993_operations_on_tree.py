...
# the tree is node 0, so parent[0] = -1 since it has no parent. You want to design
# a data structure that allows users to lock, unlock, and upgrade nodes in the
# tree.
# 
# The data structure should support the following functions:
#         
#   * Lock: Locks the given node for the given user and prevents other users
#     from locking the same node. You may only lock a node using this function if the
#     node is unlocked.
#   * Unlock: Unlocks the given node for the given user. You may only unlock a
#     node using this function if it is currently locked by the same user.
...
#   * lock(int num, int user) returns true if it is possible for the user with
#     id user to lock the node num, or false otherwise. If it is possible, the node
#     num will become locked by the user with id user.
#   * unlock(int num, int user) returns true if it is possible for the user
#     with id user to unlock the node num, or false otherwise. If it is possible, the
#     node num will become unlocked.
#   * upgrade(int num, int user) returns true if it is possible for the user
#     with id user to upgrade the node num, or false otherwise. If it is possible, the
...
# least one locked descendant (node 4).
...
# Solution: https://youtu.be/qK4PtjrVD0U
...
def main():
    locking_tree = LockingTree([-1, 0, 0, 1, 1, 2, 2])
    result = locking_tree.lock(2, 2)
    print(result) # True

    result = locking_tree.unlock(2, 3)
    print(result) # False

    result = locking_tree.unlock(2, 2)
    print(result) # True

    result = locking_tree.lock(4, 5)
    print(result) # True

    result = locking_tree.upgrade(0, 1)
    print(result) # True

    result = locking_tree.lock(0, 1)
    print(result) # False
