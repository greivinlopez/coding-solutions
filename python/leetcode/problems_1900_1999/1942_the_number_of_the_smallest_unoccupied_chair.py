...
# Solution: https://youtu.be/LqhxcaCctCc
...
def smallest_chair(times, targetFriend):
    ...
    for i in indexes:
        ...
        while used_chairs and used_chairs[0][0] <= arrive_time:
            leave_time_prev, chair_prev = heapq.heappop(used_chairs)
            heapq.heappush(available_chairs, chair_prev)

        chair = heapq.heappop(available_chairs)
        heapq.heappush(used_chairs, (leave_time, chair))
...
def main():
    result = smallest_chair(times = [[1,4],[2,3],[4,6]], targetFriend = 1)
    print(result) # 1

    result = smallest_chair(times = [[3,10],[1,5],[2,6]], targetFriend = 0)
    print(result) # 2
