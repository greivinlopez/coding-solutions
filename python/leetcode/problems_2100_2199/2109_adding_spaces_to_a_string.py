...
#     spaces before 'Y' and 'C', which are at indices 5 and 9 respectively. Thus, we
...
# Solution: https://youtu.be/jziDtPZJ4fw
...
def add_spaces(s, spaces):
    ...
    return "".join(res)
    # Time: O(n + m) n = length of the string, m = length of the spaces list 
    # Space: O(n + m)


def main():
    result = add_spaces(s = "LeetcodeHelpsMeLearn", spaces = [8,13,15])
    print(result) # "Leetcode Helps Me Learn"

    result = add_spaces(s = "icodeinpython", spaces = [1,5,7,9])
    print(result) # "i code in py thon"

    result = add_spaces(s = "spacing", spaces = [0,1,2,3,4,5,6])
    print(result) # " s p a c i n g"
