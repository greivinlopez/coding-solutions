...
# one at a time.
# 
# You are given a string keysPressed of length n, where keysPressed[i] was the iᵗʰ
# key pressed in the testing sequence, and a sorted list releaseTimes, where
# releaseTimes[i] was the time the iᵗʰ key was released. Both arrays are
# 0-indexed. The 0ᵗʰ key was pressed at the time 0, and every subsequent key was
...
# The iᵗʰ keypress had a duration of releaseTimes[i] - releaseTimes[i - 1], and
# the 0ᵗʰ keypress had a duration of releaseTimes[0].
# 
...
# Solution: https://algo.monster/liteproblems/1629
# Credit: AlgoMonster
def slowest_key(releaseTimes, keysPressed):
    # Initialize the answer with the first key pressed
    the_slowest_key = keysPressed[0]
    # Initialize the maximum duration with the first key's release time
    max_duration = releaseTimes[0]
    
    # Iterate through the remaining keys starting from index 1
    for i in range(1, len(keysPressed)):
        # Calculate the duration for the current key press
        # Duration = current release time - previous release time
        current_duration = releaseTimes[i] - releaseTimes[i - 1]
        
        # Update the slowest key if:
        # 1. Current duration is strictly greater than max duration, OR
        # 2. Current duration equals max duration AND current key is lexicographically larger
        if current_duration > max_duration or (current_duration == max_duration and ord(keysPressed[i]) > ord(the_slowest_key)):
            max_duration = current_duration
            the_slowest_key = keysPressed[i]
    
    # Return the key with the longest duration (or lexicographically largest if tied)
    return the_slowest_key
    # Time: O(n)
    # Space: O(1)


def main():
    result = slowest_key(releaseTimes = [9,29,49,50], keysPressed = "cbcd")
    print(result) # "c"

    result = slowest_key(releaseTimes = [12,23,36,46,62], keysPressed = "spuda")
    print(result) # "a"
