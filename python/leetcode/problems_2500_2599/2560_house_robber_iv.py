...
# Solution: https://youtu.be/OHZqAc6h3Us
...
def min_capability(nums, k):
    ...
    # m = range of values in nums (max(nums) - min(nums))


def main():
    result = min_capability(nums = [2,3,5,9], k = 2)
    print(result) # 5

    result = min_capability(nums = [2,7,9,3,1], k = 2)
    print(result) # 2
