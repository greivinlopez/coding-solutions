...
#         
#   1. The code must be wrapped in a valid closed tag. Otherwise, the code is
#      invalid.
...
#   4. A valid TAG_CONTENT may contain other valid closed tags, cdata and any
...
#      and vice versa. However, you also need to consider the issue of unbalanced when
#      tags are nested.
#   6. A < is unmatched if you cannot find a subsequent >. And when you find
#      a < or </, all the subsequent characters until the next > should be parsed as
#      TAG_NAME (not necessarily valid).
#   7. The cdata has the following format : <![CDATA[CDATA_CONTENT]]>. The
#      range of CDATA_CONTENT is defined as the characters between <![CDATA[ and the
#      first subsequent ]]>.
#   8. CDATA_CONTENT may contain any characters. The function of cdata is to
#      forbid the validator to parse CDATA_CONTENT, so even it has some characters that
...
# Solution: https://algo.monster/liteproblems/591
# Credit: AlgoMonster
def is_valid(code):
    def is_valid_tag(tag: str) -> bool:
        """Check if a tag name is valid (1-9 uppercase letters)"""
        return 1 <= len(tag) <= 9 and all(char.isupper() for char in tag)
    
    tag_stack = []  # Stack to track open tags
    index = 0
    code_length = len(code)
    
    while index < code_length:
        # If we're not at the start and stack is empty, code is not wrapped in tags
        if index > 0 and not tag_stack:
            return False
        
        # Check for CDATA section: <![CDATA[...]]>
        if code[index:index + 9] == '<![CDATA[':
            # Find the closing ]]> for CDATA
            closing_index = code.find(']]>', index + 9)
            if closing_index < 0:
                return False  # CDATA section not properly closed
            index = closing_index + 2  # Move past ]]>
            
        # Check for closing tag: </TAG>
        elif code[index:index + 2] == '</':
            start_tag_name = index + 2
            closing_bracket = code.find('>', start_tag_name)
            if closing_bracket < 0:
                return False  # No closing bracket found
            
            tag_name = code[start_tag_name:closing_bracket]
            # Validate closing tag matches the most recent opening tag
            if not is_valid_tag(tag_name) or not tag_stack or tag_stack.pop() != tag_name:
                return False
            index = closing_bracket
            
        # Check for opening tag: <TAG>
        elif code[index] == '<':
            start_tag_name = index + 1
            closing_bracket = code.find('>', start_tag_name)
            if closing_bracket < 0:
                return False  # No closing bracket found
            ...
            if not is_valid_tag(tag_name):
                return False
            tag_stack.append(tag_name)  # Push opening tag to stack
            index = closing_bracket
...
def main():
    result = is_valid(code = "<DIV>This is the first line <![CDATA[<div>]]></DIV>")
    print(result) # True

    result = is_valid(code = "<DIV>>>  ![cdata[]] <![CDATA[<div>]>]]>]]>>]</DIV>")
    print(result) # True

    result = is_valid(code = "<A>  <B> </A>   </B>")
    print(result) # False
