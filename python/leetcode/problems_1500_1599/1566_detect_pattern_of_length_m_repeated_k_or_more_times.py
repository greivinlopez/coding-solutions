...
# Output: true
# 
# Explanation: The pattern (4) of length 1 is repeated 4 consecutive times. Notice
...
# Solution: https://algo.monster/liteproblems/1566
# Credit: AlgoMonster
def contains_pattern(arr, m, k):
    # Early return if array is too small to contain the pattern
    if len(arr) < m * k:
        return False
    
    # Initialize counter for consecutive matches and calculate target matches needed
    consecutive_matches = 0
    # We need (k-1)*m matches because if we have k repetitions of pattern of length m,
    # there are (k-1)*m positions where arr[i] == arr[i-m]
    target_matches = (k - 1) * m
    
    # Start from index m and compare each element with element m positions before
    for i in range(m, len(arr)):
        if arr[i] == arr[i - m]:
            # Elements match the pattern, increment counter
            consecutive_matches += 1
            
            # Check if we've found enough consecutive matches
            if consecutive_matches == target_matches:
                return True
        else:
...
def main():
    result = contains_pattern(arr = [1,2,4,4,4,4], m = 1, k = 3)
    print(result) # True

    result = contains_pattern(arr = [1,2,1,2,1,1,1,3], m = 2, k = 2)
    print(result) # True

    result = contains_pattern(arr = [1,2,1,2,1,3], m = 2, k = 3)
    print(result) # False
