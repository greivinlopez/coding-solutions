...
# falls immediately out of the plank. (i.e., We can say that at t = 4.0000000001,
# there are no ants on the plank).
# 
# Example 2:
# 
...
def get_last_moment(n, left, right):
    ...
    # Time: O(l + r)
    # Space: O(1)
    # l = length of the left array
    # r = length of the right array


def main():
    result = get_last_moment(n = 4, left = [4,3], right = [0,1])
    print(result) # 4

    result = get_last_moment(n = 7, left = [], right = [0,1,2,3,4,5,6,7])
    print(result) # 7

    result = get_last_moment(n = 7, left = [0,1,2,3,4,5,6,7], right = [])
    print(result) # 7
