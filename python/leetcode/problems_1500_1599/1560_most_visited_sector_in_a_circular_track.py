...
# track, the marathon consists of m rounds. The iᵗʰ round starts at sector
...
# Solution: https://algo.monster/liteproblems/1560
# Credit: AlgoMonster
def most_visited(n, rounds):
    # Get the starting and ending sectors
    start_sector = rounds[0]
    end_sector = rounds[-1]
    
    # Case 1: Normal path (start <= end)
    # The most visited sectors are simply from start to end
    if start_sector <= end_sector:
        return list(range(start_sector, end_sector + 1))
    
    # Case 2: Wrap-around path (start > end)
    # The race wraps around the track, so most visited sectors are:
    # - From sector 1 to end_sector (first part)
    # - From start_sector to sector n (second part)
    most_visited_sectors = list(range(1, end_sector + 1)) + list(range(start_sector, n + 1))
...
def main():
    result = most_visited(n = 4, rounds = [1,3,1,2])
    print(result) # [1, 2]

    result = most_visited(n = 2, rounds = [2,1,2,1,2,1,2,1,2])
    print(result) # [2]

    result = most_visited(n = 7, rounds = [1,3,5,7])
