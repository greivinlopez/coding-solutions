...
# 
# Explanation: There are a total of 9 spaces and 4 words. We can evenly divide the
# 9 spaces between the words: 9 / (4-1) = 3 spaces.
# 
# Example 2:
# 
...
# Solution: https://algo.monster/liteproblems/1592
# Credit: AlgoMonster
def reorder_spaces(text):
    # Count total number of spaces in the text
    total_spaces = text.count(" ")
    
    # Split text into words (automatically removes all spaces)
    words = text.split()
    
    # Special case: if only one word exists, append all spaces at the end
    if len(words) == 1:
        return words[0] + " " * total_spaces
    
    # Calculate spaces between words and remaining spaces
    # spaces_between: number of spaces to place between each pair of words
    # remaining_spaces: leftover spaces to append at the end
    spaces_between, remaining_spaces = divmod(total_spaces, len(words) - 1)
    
    # Join words with calculated spaces between them and append remaining spaces
    return (" " * spaces_between).join(words) + " " * remaining_spaces
    # Time: O(n)
    # Space: O(n)


def main():
    result = reorder_spaces(text = "  this   is  a sentence ")
    print(result) # "this   is   a   sentence"

    result = reorder_spaces(text = " practice   makes   perfect")
    print(result) # "practice   makes   perfect"
