...
#
# On a 2D plane, there are n points with integer coordinates points[i] = [xᵢ, yᵢ].
...
def min_time_to_visit_all_points_alt(points):
...
def main():
    result = min_time_to_visit_all_points(points = [[1,1],[3,4],[-1,0]])
    print(result) # 7

    result = min_time_to_visit_all_points(points = [[3,2],[-2,2]])
    print(result) # 5
