...
# Solution: https://algo.monster/liteproblems/1250
# Credit: AlgoMonster
def is_good_array(nums):
    # Calculate the GCD of all numbers in the array
    # If GCD equals 1, then linear combination can produce 1
...
# Simpler Solution
def is_good_array_short(nums):
    return gcd(*nums) == 1


def main():
    result = is_good_array(nums = [12,5,7,23])
    print(result) # True

    result = is_good_array(nums = [29,6,10])
    print(result) # True

    result = is_good_array(nums = [3,6])
    print(result) # False
