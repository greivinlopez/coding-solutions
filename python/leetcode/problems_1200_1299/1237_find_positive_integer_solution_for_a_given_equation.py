...
#
# Given a callable function f(x, y) with a hidden formula and a value z, reverse
# engineer the formula and return all positive integer pairs x and y where f(x,y)
# == z. You may return the pairs in any order.
# 
...
# Solution: https://algo.monster/liteproblems/1237
# Credit: AlgoMonster
...
class Solution:
    def findSolution(self, customfunction: 'CustomFunction', z: int) -> List[List[int]]:
        ...
        Since f(x, y) is strictly increasing with respect to both x and y,
        we can use binary search to find valid y values for each x.
      
        Args:
            customfunction: Custom function object with method f(x, y)
            z: Target value to find
          
        Returns:
            List of [x, y] pairs where f(x, y) == z
        """
        result = []
      
        # Try each possible x value from 1 to z
        for x in range(1, z + 1):
            # Binary search for y where f(x, y) == z
            # Since f is increasing, we search in range [1, z]
            left, right = 1, z
