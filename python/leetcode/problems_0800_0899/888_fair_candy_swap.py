# -----------------------
# 888. Fair Candy Swap 🍬
# -----------------------

# Problem: https://leetcode.com/problems/fair-candy-swap
#
# Alice and Bob have a different total number of candies. You are given two
# integer arrays aliceSizes and bobSizes where aliceSizes[i] is the number of
# candies of the iᵗʰ box of candy that Alice has and bobSizes[j] is the number of
# candies of the jᵗʰ box of candy that Bob has.
# 
# Since they are friends, they would like to exchange one candy box each so that
# after the exchange, they both have the same total amount of candy. The total
# amount of candy a person has is the sum of the number of candies in each box
# they have.
# 
# Return an integer array answer where answer[0] is the number of candies in the
# box that Alice must exchange, and answer[1] is the number of candies in the box
# that Bob must exchange. If there are multiple answers, you may return any one of
# them. It is guaranteed that at least one answer exists.
# 
# Example 1:
# 
# Input: aliceSizes = [1,1], bobSizes = [2,2]
# Output: [1,2]
# 
# Example 2:
# 
# Input: aliceSizes = [1,2], bobSizes = [2,3]
# Output: [1,2]
# 
# Example 3:
# 
# Input: aliceSizes = [2], bobSizes = [1,3]
# Output: [2,3]
# 
# 
# Constraints:
#         1 <= aliceSizes.length, bobSizes.length <= 10⁴
#         1 <= aliceSizes[i], bobSizes[j] <= 10⁵
#         Alice and Bob have a different total number of candies.
#         There will be at least one valid answer for the given input.


# Solution: https://algo.monster/liteproblems/888
# Credit: AlgoMonster
def fair_candy_swap(aliceSizes, bobSizes):
    # Calculate the difference in total candies between Alice and Bob
    # Divide by 2 to find the amount that needs to be balanced
    difference = (sum(aliceSizes) - sum(bobSizes)) // 2
    
    # Create a set of Bob's candy sizes for O(1) lookup
    bob_sizes_set = set(bobSizes)
    
    # Iterate through Alice's candy boxes
    for alice_candy in aliceSizes:
        # Calculate what Bob's candy size should be for a fair swap
        # Alice gives alice_candy and receives bob_candy
        # After swap: Alice loses alice_candy and gains bob_candy
        # The net change for Alice is (bob_candy - alice_candy)
        # This should equal -difference to balance the totals
        bob_candy = alice_candy - difference
        
        # Check if Bob has a candy box of the required size
        if bob_candy in bob_sizes_set:
            return [alice_candy, bob_candy]
    # Time: O(m + n)
    # Space: O(n)


def main():
    result = fair_candy_swap(aliceSizes = [1,1], bobSizes = [2,2])
    print(result) # [1,2]

    result = fair_candy_swap(aliceSizes = [1,2], bobSizes = [2,3])
    print(result) # [1,2]

    result = fair_candy_swap(aliceSizes = [2], bobSizes = [1,3])
    print(result) # [2,3]

if __name__ == "__main__":
    main()
