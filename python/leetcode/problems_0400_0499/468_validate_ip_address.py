...
# Solution: https://leetcode.com/problems/validate-ip-address/solutions/3278850/468-solution-with-step-by-step-explanation
# Credit: Marlen -> https://leetcode.com/u/Marlen09/
def valid_ip_address(queryIP):
    if "." in queryIP:
        ...
        if len(segments) != 8:
            return "Neither"
        for segment in segments:
            if not segment or len(segment) > 4 or not all(c in hexdigits for c in segment):
                return "Neither"
        return "IPv6"
    return "Neither"
    # Time: O(n)
...
def main():
    result = valid_ip_address("172.16.254.1")
    print(result) # "IPv4"

    result = valid_ip_address("2001:0db8:85a3:0:0:8A2E:0370:7334")
    print(result) # "IPv6"

    result = valid_ip_address("256.256.256.256")
