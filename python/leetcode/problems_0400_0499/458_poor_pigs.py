...
# poisonous. To figure out which one is poisonous, you feed some number of (poor)
...
#     the chosen buckets simultaneously and will take no time. Each pig can feed from
...
# At time 0, feed the first pig buckets 1 and 2, and feed the second pig buckets 2
# and 3.
...
# bucket 4.
# At time 30, one of the two pigs must die, and the poisonous bucket is the one it
# was fed.
# 
# 
# Constraints:
...
def poor_pigs(buckets, minutesToDie, minutesToTest):
    ...
    # t = the ratio minutesToTest / minutesToDie


def main():
    result = poor_pigs(buckets = 4, minutesToDie = 15, minutesToTest = 15)
    print(result) # 2

    result = poor_pigs(buckets = 4, minutesToDie = 15, minutesToTest = 30)
    print(result) # 2
