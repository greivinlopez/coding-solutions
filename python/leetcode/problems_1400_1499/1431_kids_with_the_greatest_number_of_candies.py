...
# Solution: https://algo.monster/liteproblems/1431
# Credit: AlgoMonster
def kids_with_candies(candies, extraCandies):
    # Find the maximum number of candies among all kids
    max_candies = max(candies)
    
    # Check if each kid can have >= max candies after receiving extra candies
    # A kid can have the greatest number if their total >= current maximum
...
def main():
    result = kids_with_candies(candies = [2,3,5,1,3], extraCandies = 3)
    print(result) # 6

    result = kids_with_candies(candies = [4,2,1,1,2], extraCandies = 1)
    print(result) # 0

    result = kids_with_candies(candies = [12,1,12], extraCandies = 10)
    print(result) # 1
