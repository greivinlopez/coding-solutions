...
# Solution: https://algo.monster/liteproblems/1476
# Credit: AlgoMonster
class SubrectangleQueries:

    def __init__(self, rectangle):
        self.matrix = rectangle
        # Store update operations as a list of tuples (row1, col1, row2, col2, new_value)
        self.update_history = []

    def updateSubrectangle(self, row1: int, col1: int, row2: int, col2: int, newValue: int):
        # Append the update operation to history
        self.update_history.append((row1, col1, row2, col2, newValue))

    def getValue(self, row: int, col: int) -> int:
        # Iterate through update history in reverse order (most recent first)
        for top_row, left_col, bottom_row, right_col, value in reversed(self.update_history):
            # Check if the current cell falls within this update's subrectangle
