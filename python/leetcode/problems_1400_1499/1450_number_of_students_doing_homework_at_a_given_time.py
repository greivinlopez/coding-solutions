...
# Solution: https://algo.monster/liteproblems/1450
# Credit: AlgoMonster
def busy_studens(startTime, endTime, queryTime):
    # Count students where queryTime falls within their homework period
    # For each student i, check if startTime[i] <= queryTime <= endTime[i]
    busy_count = sum(
        start <= queryTime <= end 
...
def main():
    result = busy_studens(startTime = [1,2,3], endTime = [3,2,7], queryTime = 4)
    print(result) # 1

    result = busy_studens(startTime = [4], endTime = [4], queryTime = 4)
    print(result) # 1
