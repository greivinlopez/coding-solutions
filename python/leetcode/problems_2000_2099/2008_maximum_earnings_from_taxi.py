...
# drive from point 1 to point n to make money by picking up passengers. You cannot
# change the direction of the taxi.
# 
# The passengers are represented by a 0-indexed 2D integer array rides, where
# rides[i] = [startᵢ, endᵢ, tipᵢ] denotes the iᵗʰ passenger requesting a ride from
# point startᵢ to point endᵢ who is willing to give a tipᵢ dollar tip.
# 
# For each passenger i you pick up, you earn endᵢ - startᵢ + tipᵢ dollars. You may
# only drive at most one passenger at a time.
# 
# Given n and rides, return the maximum number of dollars you can earn by picking
# up the passengers optimally.
# 
# Note: You may drop off a passenger and pick up a different passenger at the same
# point.
# 
# Example 1:
# 
...
def main():
    result = max_taxi_earnings(n = 5, rides = [[2,5,4],[1,5,1]])
    print(result) # 7

    result = max_taxi_earnings(n = 20, rides = [[1,6,1],[3,10,2],[10,12,3],[11,12,2],[12,15,2],[13,18,1]])
    print(result) # 20
