...
# Solution: https://youtu.be/UddDgt52h9g
...
def nodes_between_critical_points(head):
    ...
    if min_dist == float("inf"): min_dist = -1
...
def main():
    head = from_list([3,1])
    result = nodes_between_critical_points(head)
    print(result) # [-1,-1]

    head = from_list([5,3,1,2,5,1,2])
    result = nodes_between_critical_points(head)
    print(result) # [1,3]

    head = from_list([1,3,2,2,3,2,2,2,7])
    result = nodes_between_critical_points(head)
    print(result) # [3,3]
