...
# string encodedText with the help of a matrix having a fixed number of rows rows.
# 
...
# The number of columns is chosen such that the rightmost column will not be empty
# after filling in originalText.
# 
...
# 
# The characters in the blue cells are appended first to encodedText, then the red
...
def decode_ciphertext(encodedText, rows):
    n = len(encodedText)
    cols = n//rows
...
def main():
    result = decode_ciphertext("ch   ie   pr", 3)
    print(result) # "cipher"

    result = decode_ciphertext("iveo    eed   l te   olc", 4)
    print(result) # "i love leetcode"

    result = decode_ciphertext("coding", 1)
    print(result) # "coding"
