...
#     maximal value that can be subtracted from the input, append that symbol to the
...
#     form representing one symbol subtracted from the following symbol, for
#     example, 4 is 1 (I) less than 5 (V): IV and 9 is 1 (I) less than 10 (X):
#     IX. Only the following subtractive forms are used: 4 (IV), 9 (IX), 40 (XL), 90
...
# Solution: https://youtu.be/ohBNdSJyLh8
...
def main():
    result = int_to_roman(3749) # "MMMDCCXLIX"
    print(result)
    result = int_to_roman(58) # "LVIII"
    print(result)
    result = int_to_roman(1994) # "MCMXCIV"
    print(result)
