...
# Solution: https://youtu.be/Akt3glAwyfY
...
# Solution: https://youtu.be/2cpihwDznaw
# Credit: Greg Hogg
# Similar Solution
def find_order_alt(numCourses, prerequisites):
    order = []
    g = defaultdict(list)
...
def main():
    result = find_order(numCourses = 2, prerequisites = [[1,0]])
    print(result) # [0,1]

    result = find_order(numCourses = 4, prerequisites = [[1,0],[2,0],[3,1],[3,2]])
    print(result) # [0,1,2,3] or [0,2,1,3]

    result = find_order(numCourses = 1, prerequisites = [])
    print(result) # [0]
